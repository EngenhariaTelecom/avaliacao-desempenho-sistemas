...
class StartTransmissionEvent(Event):
    ...
    def processing_event(self, sim):
        ...
        # ── 1. CHECAGEM DE CANAL OCUPADO (Sinal já chegou aqui) ──
        # Se o outro está transmitindo E o sinal dele já cruzou o tempo de propagação:
        if other['transmitting'] and sim.current_time >= (other['tx_start_time'] + PROP_DELAY):
            # Conforme o enunciado: "fazem backoff aleatório se detectarem canal ocupado"
            # Não conta colisão! Apenas vai para o estado de Espera (Backoff)
            sim.schedule(BackoffEvent(sim.current_time, self.station_id))
            return

        # ── 2. CHECAGEM DE COLISÃO (Janela de vulnerabilidade) ──
        # Se o outro está transmitindo MAS o sinal ainda não chegou aqui:
        elif other['transmitting'] and sim.current_time < (other['tx_start_time'] + PROP_DELAY):
            sim.total_collisions += 1
            me['collisions']     += 1
            other['collisions']  += 1

            # Ambas abortam a transmissão
            me['transmitting']    = False
            other['transmitting'] = False

            # Ambas entram em backoff por colisão
            sim.schedule(BackoffEvent(sim.current_time, self.station_id))
        ...
        # ── 3. CANAL LIVRE ──
            ...
            # Agenda o fim de transmissão de sucesso
